...
//It is the same as PELLCAPP, but uses GuessVAL instead of Guess
public class PELLCAPPVAL implements Algorithm
{
    public PELLCAPPVAL(double _beta, int[] _ki) {
        beta = _beta;
        ki = _ki;
    ...
    public void update(Point p, int time)
    {
        ...
        int lastIndex = (int) Math.ceil(Math.log(M_t) / Math.log(1 + beta)); //as delta is 2, remains only M_t
        ...
        if (guesses.isEmpty()) {
            for (int i = firstIndex; i <= lastIndex; i++) {
                ...
                guesses.put(i, new GuessVAL(Math.pow((1 + beta), i), ki, RV));
            }
        } else {
    ...
    private final TreeMap<Integer, GuessVAL> guesses = new TreeMap<>();
