package it.unidp.dei.OURS.PELL;

import it.unidp.dei.Algorithm;
import it.unidp.dei.OURS.Utils.Diameter.PELLDiameter;
...
//Algorithm of Pellizzoni oblivious to the aspect ratio thanks to the PELLDiameter algorithm.
//It uses Guess class.
public class PELLOBL implements Algorithm
{
    public PELLOBL(int[] _ki, double _delta, double _beta) {
        beta = _beta;
        delta = _delta;
        ki = _ki;
    ...
    public void update(Point p, int time)
    {
        ...
        if (guesses.isEmpty()) {
            for (int i = firstIndex; i <= lastIndex; i++) {
                ...
                TreeMap<Point, Point> R = new TreeMap<>();
                R.put(last_points.getLast(), last_points.getLast());

                guesses.put(i, new PELLGuess(Math.pow((1 + beta), i), delta, ki, RV, R));
            }
        } else {
        ...
        for(PELLGuess g : guesses.values()) {
            g.update(p, time);
        }
    }
    ...
    private final TreeMap<Integer, PELLGuess> guesses = new TreeMap<>();
