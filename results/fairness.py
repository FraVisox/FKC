# SCRIPT FOR READING fairness_price.csv THAT CONTAINS DATA FOR DATASETS HIGGS, COVTYPE AND PHONE
# WITH AND WITHOUT FAIRNESS CONSTRAINT
...
# File to read from
file_name = "experiments_results/fairness_price.csv"
output_file = "graphs/FAIRNESS"

COLORS = sns.color_palette()
PALETTE = {
    "OursOblivious 0.5": COLORS[0],
    "Ours 0.5": COLORS[1],
    "NotFairOblivious 0.5": COLORS[2],
    "NotFair 0.5": COLORS[3],
    "OursOblivious 2.0": COLORS[4],
    "Ours 2.0": COLORS[5],
    "NotFairOblivious 2.0": COLORS[6],
    "NotFair 2.0": COLORS[7],
}
...
def read_and_plot(output_file_path):
    ...
    df = df.with_columns(
        ...
        pl.col("algorithm").str.replace("PELLOBL05", "NotFairOblivious 0.5"),
    ).with_columns(
        pl.col("algorithm").str.replace("PELL05", "NotFair 0.5")
    ).with_columns(
    ...
    for graph in y_axis:
        ...
        hue_order = ["OursOblivious 0.5", "NotFairOblivious 0.5",
                     "Ours 0.5", "NotFair 0.5", 
                     "OursOblivious 2.0", "NotFairOblivious 2.0",
                      "Ours 2.0", "NotFair 2.0"]
        g.map_dataframe(
