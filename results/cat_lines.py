...
replace_commas = True
graph_name = "categories_lines"
input_file_name = "categories"
y_axis = ["update", "query", "memory", "ratio"]
color = "algorithm"

# File to read from
datasets = ["phones"]#["phones", "higgs", "covtype"]
file_names = ["categories_phones"]#[f"{input_file_name}_jones_{dataset}" for dataset in datasets]
...
def load(file, basedir="experiments_results/"):
    ...
    df = df.select(
        "wsize", "algorithm", "update", "query", "radius", "ratio", "memory", "dataset", "categories"
    )
    return filter(df)
...
def read_and_plot_bar(output_file_path):
    ...
    for graph in y_axis:
        ...
        g.set_xlabels("categories", usetex=True)
